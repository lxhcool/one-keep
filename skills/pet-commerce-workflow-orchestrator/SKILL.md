...
description: 串联宠物商城端到端交付流程，按固定顺序调用布局分析、需求编写、思考审查、DDD 架构、业务实现、代码审查与测试，并在每次执行中生成完整且可追踪的 Markdown 产物。用于用户要求整条工作流协同推进与文档沉淀时触发。
---

# 宠物商城工作流编排

为每次功能迭代建立一套可追踪、可复盘、可执行的文档化交付流程。

## 工作流契约

### 1. 初始化一次运行目录

- 运行 `scripts/init_workflow_docs.sh <workflow-id> [page-slug ...]`。
...
4. `$ddd-architecture-designer`
5. `$business-implementation-builder`
6. `$code-review-test-auditor`
...
- 架构阶段必须生成：`04-architecture/page-architecture-mapping.md`。
- 需求编号在以下文档中保持一致：
- `02-requirements/*.md`
- `03-thinking/*.md`
- `04-architecture/*.md`
...
- 使用 PRD、交互文档与思考审查结果执行 `$ddd-architecture-designer`。
- 输出写入：
- `04-architecture/ddd-overview.md`
- `04-architecture/page-architecture-mapping.md`
- `04-architecture/contracts.md`
- 阶段结束后等待用户确认，再进入阶段 5。
...
- `skills/ddd-architecture-designer/references/architecture-template.md`
