...
| 项目 | 信息 |
|---|---|
| **服务器** | `175.24.199.30` (root, SSH 密钥登录) |
| **域名** | `https://liqing.eatdesk.net` |
| **部署路径** | `/opt/one-keep-server/` |
| **部署方式** | Docker Compose |
| **容器端口** | 3002 |
| **数据库** | SQLite (持久化到 `/opt/one-keep-server/data/dev.db`) |
| **SSL** | 宝塔面板管理 (Let's Encrypt) |
| **Nginx** | 宝塔面板管理，反向代理到 `http://127.0.0.1:3002` |

### ⚠️ 数据库备份与迁移注意

服务器上存在多个历史数据库文件，**务必不要搞混**：

| 文件路径 | 说明 | 用户数 | 用途 |
|---|---|---|---|
| `/opt/one-keep-server/data/dev.db` | **当前线上数据库** | 2 (lxhcool, xjh) | Docker 容器挂载使用，线上真实数据 |
| `/www/wwwroot/one-keep/server/data/dev.db` | 旧 Docker 部署的数据库 | 2 | 来源同上，已迁移到新路径 |
| `/www/wwwroot/one-keep/server/prisma/dev.db` | 本地开发测试数据 | 4 (含测试用户) | **不是线上数据，不要用！** |

关键要点：

- **线上数据库** = `/opt/one-keep-server/data/dev.db`，Docker 容器通过 volume 映射使用
- 更新部署时 `rsync` 已排除 `data/` 目录，**不会覆盖线上数据库**
- 如果需要恢复数据，必须从 `/www/wwwroot/one-keep/server/data/dev.db`（旧 Docker）恢复，**不要用 prisma/dev.db**
- 重要操作前先备份：`cp /opt/one-keep-server/data/dev.db /opt/one-keep-server/data/dev.db.bak`
...
# 2. 进入项目目录，上传最新代码（从本地 rsync）
# 在本地执行：
rsync -avz --exclude='node_modules' --exclude='dist' --exclude='.env' --exclude='dev.db' --exclude='data' \
  -e "ssh" ./server/ root@175.24.199.30:/opt/one-keep-server/

# 3. 在服务器上重新构建并启动
ssh root@175.24.199.30 "cd /opt/one-keep-server && docker compose up -d --build"
...
- 服务器：`119.45.243.103`
- 站点目录：`/www/wwwroot/liqing.eatdesk.net/client`
- 服务端目录：`/www/wwwroot/liqing.eatdesk.net/server`
- PM2 进程名：`one-keep-server`
- 线上域名：`https://liqing.eatdesk.net`
...
```bash
# 查看 Docker 容器状态
ssh root@175.24.199.30 "cd /opt/one-keep-server && docker compose ps"

# 查看 Docker 容器日志
ssh root@175.24.199.30 "docker logs one-keep-server --tail 50"

# 重启容器
ssh root@175.24.199.30 "cd /opt/one-keep-server && docker compose restart"

# 本机健康检查
ssh root@175.24.199.30 "curl http://127.0.0.1:3002/api/health"
