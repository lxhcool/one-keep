...
| 分析页面 | OneKeep 首页（深色 M5Wes）/ OneKeep 首页 - Light（浅色 IY3QK）；交互状态帧：余额隐藏（wWC5E）、FAB 快速记账弹层（zXAee）、交易项详情（vmR2y） |
...
| 切换金额显隐 | 点击余额卡右上角眼睛图标 | 眼睛切换为 eye-off、金额变为 ¥ •••••••• 掩码 | P1 | 设计稿已补充隐藏态帧 wWC5E |
...
| 查看单条交易详情 | 点击交易卡片 | 弹出底部详情弹层（含分类图标、标题、金额、日期/商家/备注、编辑/删除） | P1 | 设计稿已补充详情弹层帧 vmR2y |
| 切换到统计页 | 点击底部“统计” | 底部导航切换选中态并进入统计页 | P0 | 统计页设计稿已存在 |
| 快速新增记账 | 点击中间 FAB | 弹出底部快速记账弹层（含支出/收入切换、金额、分类快选、确认按钮） | P0 | 设计稿已补充 FAB 弹层帧 zXAee |
